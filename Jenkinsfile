pipeline {
    ...
    environment {
        ...
        // Local variables
        DOCKER_IMAGE_TAG       = "mydnacodes/analysis-result"
        DOCKER_IMAGE           = ""
        COMMIT_AUTHOR          = ""
        COMMIT_MESSAGE         = ""
        PROJECT_VERSION        = ""
            ...
            steps {
                script {
                    ...
                    sed -e 's+{{IMAGE_NAME}}+$DOCKER_IMAGE_TAG:$PROJECT_VERSION+g' \
                        ...
                        .kube/analysis-result.yaml > .kube/analysis-result.tmp
                    """
                    sh "mv -f .kube/analysis-result.tmp .kube/analysis-result.yaml"

                    sh """ \
                    sed -e 's+{{NAMESPACE}}+$environment.namespace+g' \
                        .kube/analysis-result-db.yaml > .kube/analysis-result-db.tmp
                    """
                    sh "mv -f .kube/analysis-result-db.tmp .kube/analysis-result-db.yaml"
                }
            }
        }
                    ...
                    try {
                        if (!(env.GIT_BRANCH.equals("prod") || env.GIT_BRANCH.equals("origin/prod"))) {
                            withKubeConfig([credentialsId: KUBERNETES_CREDENTIALS]) {
                                sh "kubectl delete deployments.apps -n mydnacodes analysis-result-app"
                            }
                        }
                    } catch (Exception e) {
